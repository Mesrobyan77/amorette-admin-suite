...
import {
  Search, Plus, Trash2, Copy, Eye, Pencil, FileImage, ChevronLeft, ChevronRight, ArrowUpDown,
} from "lucide-react";
...
function TemplatesListPage() {
  ...
  const allOnPageSelected = pageItems.length > 0 && pageItems.every((t) => selected.has((t.id || t._id) as string));
  ...
  const doBulkDelete = async () => {
    ...
    await toast.promise(
      Promise.allSettled(ids.map((id) => templatesApi.remove(id))),
      { loading: `Deleting ${ids.length}…`, success: "Bulk delete complete", error: "Some deletions failed" }
    );
    setTemplates((prev) => (prev || []).filter((x) => !ids.includes((x.id || x._id) as string)));
    setSelected(new Set());
  };

  const doDuplicate = async (t: Template) => {
    const fd = new FormData();
    fd.append("name", `${t.name} (Copy)`);
    if (t.category) fd.append("category", t.category);
    if (t.description) fd.append("description", t.description);
    fd.append("basePrice", String(t.basePrice));
    fd.append("currency", t.currency || "֏");
    if (t.musicTitle) fd.append("musicTitle", t.musicTitle);
    fd.append("features", JSON.stringify(t.features || []));
    try {
      const { data } = await toast.promise(templatesApi.create(fd), {
        loading: "Duplicating…", success: "Template duplicated", error: "Duplicate failed",
      });
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{filtered.length} of {templates?.length ?? 0} templates</p>
        </div>
        <Link to="/admin/templates/create">
          <Button variant="gold" fullWidth><Plus className="h-4 w-4" /> Create Template</Button>
        </Link>
      </div>
      ...
      {templates === null ? (
        ...
        <Card className="text-center py-16">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {search || category !== "all" ? "Try adjusting your filters." : "Create your first template to get started."}
          </p>
          <div className="mt-5 flex justify-center">
            <Link to="/admin/templates/create"><Button variant="gold"><Plus className="h-4 w-4" /> Create Template</Button></Link>
          </div>
        </Card>
      ) : (
        <>
                ...
                >
                  <div className="relative aspect-[4/3] bg-muted overflow-hidden">
                    {t.mainImage ? (
                      ...
                      <div className="h-full w-full flex items-center justify-center text-muted-foreground"><FileImage className="h-10 w-10" /></div>
                    )}
                    <label className="absolute top-2 left-2 h-8 w-8 rounded-xl bg-black/50 backdrop-blur flex items-center justify-center cursor-pointer">
                      <input type="checkbox" checked={isSelected} onChange={() => toggleOne(id)} className="h-4 w-4 accent-[var(--gold)]" />
                    </label>
                    {t.category && (
                      <div className="absolute top-2 right-2"><Badge variant="gold">{t.category}</Badge></div>
                    )}
                  </div>
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <p className="text-sm font-semibold text-gradient-gold whitespace-nowrap">{formatCurrency(t.basePrice, t.currency || "֏")}</p>
                    </div>
                    <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground">
                      <span className="inline-flex items-center gap-1"><Eye className="h-3.5 w-3.5" /> {t.views || 0}</span>
                      <span className="inline-flex items-center gap-1">★ {t.rating ?? "—"}</span>
                    </div>
                    <div className="mt-4 grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
                      <Button size="sm" variant="outline" onClick={() => navigate({ to: "/admin/templates/$id", params: { id } })}>
                        <Eye className="h-3.5 w-3.5" /> View
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => navigate({ to: "/admin/templates/edit/$id", params: { id } })}>
                        <Pencil className="h-3.5 w-3.5" /> Edit
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => doDuplicate(t)}>
